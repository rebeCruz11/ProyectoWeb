...
const handleRegister = async (e) => {
    ...
    try {
        ...
        // Si los datos están en response.data.user, ajusta lo siguiente:
        localStorage.setItem('userID', response.data.userID || response.data.user?.userID);
        localStorage.setItem('userName', response.data.nombre || response.data.user?.nombre);
        localStorage.setItem('email', response.data.email || response.data.user?.email);


        navigate('/plans'); // Redirige al formulario de perfil
    } catch (error) {
        console.error('Error al registrarse:', error);
        Swal.fire('Error', 'Error al registrarse. Intenta de nuevo.', 'error');
    }
...
const handleLogin = async (e) => {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            Swal.fire('Bienvenido', 'Inicio de sesión exitoso', 'success').then(() => {
                navigate('/inicioApp'); // Redirige a la página principal
            });
        }
    } catch (error) {
        console.error('Error al iniciar sesión:', error);
